...
**Live Detection — push-экран вместо tab.** Стартовый экран приложения теперь Home Hub (плитки режимов). Live Detection открывается как push-экран с кнопкой назад, bottom nav скрыт. Layout и поведение экрана не изменились.

Bottom Nav layout обновлён:
```
├────────┬────────┬────────┤
│ 🏠 Home│ Истор. │ Настр. │  ← Bottom Navigation Bar (скрыт в push-экранах)
└────────┴────────┴────────┘
```
